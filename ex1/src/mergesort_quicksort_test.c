...
// Floating Point
void mergesort_fp_test_zero_elements() {
    float arr[0];
    merge_sort(arr, 0, sizeof(float), compare_data_fp);
    TEST_ASSERT_EQUAL(0, sizeof(arr));
}

void mergesort_fp_test_one_element() {
    float arr[] = {42.42};
    merge_sort(arr, 1, sizeof(float), compare_data_fp);
    TEST_ASSERT_EQUAL_FLOAT(42.42, arr[0]); 
}

void mergesort_fp_test_four_elements() {
    float arr[] = {4.4, 2.2, 1.1, 3.3};
    float expected[] = {1.1, 2.2, 3.3, 4.4};
    merge_sort(arr, 4, sizeof(float), compare_data_fp);
...
// Floating Point
void quicksort_fp_test_zero_elements() {
    float arr[0];
    quick_sort(arr, 0, sizeof(float), compare_data_fp);
    TEST_ASSERT_EQUAL(0, sizeof(arr));
}

void quicksort_fp_test_one_element() {
    float arr[] = {42.42};
    quick_sort(arr, 1, sizeof(float), compare_data_fp);
    TEST_ASSERT_EQUAL_FLOAT(42.42, arr[0]); 
}

void quicksort_fp_test_four_elements() {
    float arr[] = {4.4, 2.2, 1.1, 3.3};
    float expected[] = {1.1, 2.2, 3.3, 4.4};
    quick_sort(arr, 4, sizeof(float), compare_data_fp);
    TEST_ASSERT_EQUAL_FLOAT_ARRAY(expected, arr, 4);
}
...
int main(){
    ...
    RUN_TEST(mergesort_fp_test_zero_elements);
    RUN_TEST(mergesort_fp_test_one_element);
    RUN_TEST(mergesort_fp_test_four_elements);
    ...
    // Floating Point tests
    RUN_TEST(quicksort_fp_test_zero_elements);
    RUN_TEST(quicksort_fp_test_one_element);
    RUN_TEST(quicksort_fp_test_four_elements);
